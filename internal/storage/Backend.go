...
type Backend interface {
	...
	// single object stored at key and returns the total size and overall MD5
	// checksum. The assembled object must then be retrievable via Open(key).
	Assemble(uploadID string, fileID string, key string, partNumbers []int) (size int64, checksum string, err error)

	// Open returns a ReadSeekCloser for the assembled object at key so that
	// HTTP range requests are supported out-of-the-box.
