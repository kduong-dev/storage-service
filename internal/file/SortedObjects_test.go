...
func TestSortedObjects(t *testing.T) {
	Convey("Given sorted objects built from inserts in arbitrary order", t, func() {
		...
		for _, object := range []*storageservice.File{
			{ID: "file-c", Key: "alpha/reports/b.html", Revision: 2},
			{ID: "file-a", Key: "alpha/reports/a.html", Revision: 1},
			{ID: "file-e", Key: "beta/reports/a.html", Revision: 1},
			{ID: "file-d", Key: "alpha/images/logo.png", Revision: 1},
			{ID: "file-b", Key: "alpha/reports/b.html", Revision: 1},
		} {
			objects.Add(object)
		}
		...
		Convey("When reading every object in one page", func() {
			...
			Convey("Then they are ordered by key, then by revision for shared keys", func() {
		...
		Convey("When an object is added after a read has sorted them", func() {
			...
			objects.Add(&storageservice.File{ID: "file-f", Key: "alpha/aaa.html", Revision: 1})
			page, _ := objects.Page(file.PageInput{Limit: 10})
		...
		Convey("When an object sharing its key with another is removed", func() {
			objects.Remove(&storageservice.File{ID: "file-b", Key: "alpha/reports/b.html", Revision: 1})
			page, _ := objects.Page(file.PageInput{Limit: 10})
		...
		Convey("When an absent object is removed", func() {
			objects.Remove(&storageservice.File{ID: "file-z", Key: "zeta/a.html", Revision: 1})
			page, _ := objects.Page(file.PageInput{Limit: 10})
		...
		Convey("When finding the index after an object", func() {
			Convey("Then it is just past the object, or where it would be inserted if absent", func() {
				So(objects.IndexAfter(&storageservice.File{ID: "file-b", Key: "alpha/reports/b.html", Revision: 1}), ShouldEqual, 3)
				So(objects.IndexAfter(&storageservice.File{ID: "file-g", Key: "alpha/reports/b.html", Revision: 3}), ShouldEqual, 4)
				So(objects.IndexAfter(&storageservice.File{ID: "file-z", Key: "zeta/a.html", Revision: 1}), ShouldEqual, 5)
			})
		})
		...
		Convey("When taking the page after the last object on a previous page", func() {
			page, hasMore := objects.Page(file.PageInput{KeyPrefix: "alpha/", After: &storageservice.File{ID: "file-a", Key: "alpha/reports/a.html", Revision: 1}, Limit: 2})
