...
func TestSortedObjects(t *testing.T) {
	Convey("Given sorted objects built from inserts in arbitrary order", t, func() {
		...
		ids := func(objects []*storageservice.FileObject) []string {
		...
		Convey("When an object is added after a read has sorted them", func() {
			...
			objects.Add(&storageservice.FileObject{ID: "file-f", Key: "alpha/aaa.html"})
			page, _ := objects.Page(file.PageInput{Limit: 10})
		...
		Convey("When an object sharing its key with another is removed", func() {
			objects.Remove(&storageservice.FileObject{ID: "file-b", Key: "alpha/reports/b.html"})
			page, _ := objects.Page(file.PageInput{Limit: 10})
		...
		Convey("When an absent object is removed", func() {
			objects.Remove(&storageservice.FileObject{ID: "file-z", Key: "zeta/a.html"})
			page, _ := objects.Page(file.PageInput{Limit: 10})
		...
		Convey("When finding the index after an object", func() {
			Convey("Then it is just past the object, or where it would be inserted if absent", func() {
				So(objects.IndexAfter(&storageservice.FileObject{ID: "file-b", Key: "alpha/reports/b.html"}), ShouldEqual, 3)
				So(objects.IndexAfter(&storageservice.FileObject{ID: "file-bb", Key: "alpha/reports/b.html"}), ShouldEqual, 3)
				So(objects.IndexAfter(&storageservice.FileObject{ID: "file-z", Key: "zeta/a.html"}), ShouldEqual, 5)
			})
		})
		...
		Convey("When taking the page after the last object on a previous page", func() {
			page, hasMore := objects.Page(file.PageInput{KeyPrefix: "alpha/", After: &storageservice.FileObject{ID: "file-a", Key: "alpha/reports/a.html"}, Limit: 2})
