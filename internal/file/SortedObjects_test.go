	...
	Convey("Given sorted objects built from inserts in arbitrary order", t, func() {
		...
		Convey("When finding the index after an object", func() {
			Convey("Then it is just past the object, or where it would be inserted if absent", func() {
				So(objects.IndexAfter(&file.Object{ID: "file-b", Key: "alpha/reports/b.html"}), ShouldEqual, 3)
				So(objects.IndexAfter(&file.Object{ID: "file-bb", Key: "alpha/reports/b.html"}), ShouldEqual, 3)
				So(objects.IndexAfter(&file.Object{ID: "file-z", Key: "zeta/a.html"}), ShouldEqual, 5)
			})
		})
